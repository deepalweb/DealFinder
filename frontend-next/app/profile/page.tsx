        ...
        <div>
          <div className="promotion-card" style={{ padding:'1.25rem' }}>
            ...
            <nav className="flex flex-col gap-1">
              {TABS.map(t => (
                <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ display:'flex', alignItems:'center', gap:'0.6rem', padding:'0.6rem 0.875rem', borderRadius:'0.625rem', border:'none', cursor:'pointer', fontSize:'0.875rem', fontWeight:500, textAlign:'left', background:activeTab===t.id?'linear-gradient(135deg,var(--primary-color),var(--primary-dark))':'transparent', color:activeTab===t.id?'#fff':'var(--text-secondary)' }}>
              ...
              <div>
                ...
                <form onSubmit={handlePasswordSave}>
                  {[['currentPassword','Current Password'],['newPassword','New Password'],['confirmPassword','Confirm New Password']].map(([k,l]) => (
                    <div key={k} style={{ marginBottom:'1rem' }}><label style={{ display:'block', fontSize:'0.85rem', fontWeight:600, marginBottom:'0.4rem', color:'var(--text-primary)' }}>{l}</label><input type="password" style={inputStyle} value={(pwForm as any)[k]} onChange={e=>setPwForm({...pwForm,[k]:e.target.value})} required /></div>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-col gap-3 mb-5">
                  {Object.keys(notifications).map(k => (
                    <label key={k} style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.875rem', borderRadius:'0.75rem', border:'1.5px solid var(--border-color)', cursor:'pointer', background:'var(--card-bg)' }}>
              ...
              <div>
                ...
                {favorites.length === 0 ? <div className="text-center py-12"><div style={{ fontSize:'2.5rem', marginBottom:'0.75rem' }}>💔</div><p style={{ color:'var(--text-secondary)' }}>No favorites yet.</p><Link href="/categories/all" className="btn btn-primary mt-3" style={{ display:'inline-flex' }}>Browse Deals</Link></div> : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {favorites.map((p:any) => (
                      <div key={p._id||p.id} style={{ border:'1.5px solid var(--border-color)', borderRadius:'0.875rem', padding:'0.875rem', display:'flex', gap:'0.875rem', alignItems:'center', background:'var(--card-bg)' }}>
