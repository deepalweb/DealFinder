...
class _HomeScreenState extends State<HomeScreen> {
        ...
        slivers: <Widget>[
          ...
          FutureBuilder<List<Promotion>>(
            ...
            builder: (context, snapshot) {
              ...
              if (_selectedFilterChip != 0 && _selectedFilterChip < _filterChipLabels.length) { // Added bounds check
                final label = _filterChipLabels[_selectedFilterChip].toLowerCase();
                if (label == 'nearby') { // Placeholder for actual nearby logic
                  // filteredPromotions = applyNearbyFilter(filteredPromotions, userLocation);
                } else if (label == 'trending') { // Placeholder for actual trending logic
                  // filteredPromotions = applyTrendingFilter(filteredPromotions);
                } else if (label != 'all') {
          ...
          SliverToBoxAdapter(
            child: FutureBuilder<List<Promotion>>(
              future: _allPromotionsFuture, // Consider a different future for actual recommendations
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return SizedBox(
                    height: 180, // Standardized height
                    child: ListView.builder( // Changed to ListView.builder for consistency
                      scrollDirection: Axis.horizontal,
                      padding: const EdgeInsets.symmetric(horizontal: 12.0),
                      itemCount: 3, // Placeholder count for shimmer
                      itemBuilder: (context, index) => Padding( // Added padding
                        padding: const EdgeInsets.only(right: 8.0),
                        child: const SizedBox(width: 260, child: DealCardShimmer()),
                      )
                    ),
                  );
                }
                ...
                // Example: take top 5 deals as recommendations
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Dismissible(
                            ...
                            child: SizedBox(
                              width: 260,
                              child: DealCard( // Assuming DealCard handles InkWell internally or you wrap it
                                promotion: promotion,
                              ),
                            ),
                          );
  ...
  Widget _buildSwipeAction(BuildContext context, IconData icon, String label, Color color) {
    return Container(
      alignment: icon == Icons.bookmark ? Alignment.centerLeft : Alignment.centerRight, // Comma added
      padding: const EdgeInsets.symmetric(horizontal: 24), // Comma added
      color: color.withOpacity(0.15), // Comma added
      child: Row(
...
class RecommendedForYouSection extends StatelessWidget {
  ...
  Widget _buildSwipeAction(BuildContext context, IconData icon, String label, Color color) {
    return Container(
      alignment: icon == Icons.bookmark ? Alignment.centerLeft : Alignment.centerRight, // Comma ADDED HERE
      padding: const EdgeInsets.symmetric(horizontal: 24), // Comma added
      color: color.withOpacity(0.15), // Comma added
      child: Row(
        mainAxisAlignment: icon == Icons.bookmark ? MainAxisAlignment.start : MainAxisAlignment.end,
