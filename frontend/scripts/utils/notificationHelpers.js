...
function generateDealRecommendations() {
  try {
    ...
    const recommendations = promotionsData.filter((promo) => {
    ...
    sort((a, b) => {
      ...
      const aIsFollowed = followedMerchantNames.includes(a.merchant.toLowerCase());
      const bIsFollowed = followedMerchantNames.includes(b.merchant.toLowerCase());
...
function sendNewMerchantDealNotifications() {
  try {
    ...
    newDeals.forEach((deal) => {
      if (!dealsByMerchant[deal.merchant]) {
        dealsByMerchant[deal.merchant] = [];
      }
      dealsByMerchant[deal.merchant].push(deal);
    });
